...
public class MusicAction extends HomeAction
{
	...
	protected Object execute(String actionId, Map params, ConversationSession session)
	{
		...
		try
		{
			...
			if ( action != null && "play".equals(action.getValue()) )
			{
				...
				{
					String info = inputStr.substring(0, target.getLocation()[0]).trim();
					int ix = info.indexOf(" ");
					if ( ix != -1 ) {
					...
					String info = inputStr.substring(0, action.getLocation()[0]).trim();
					int ix = info.indexOf(" ");
					if ( ix != -1 ) {
						String artist = info.substring(0, ix);
						String title = info.substring(ix+1);
			...
			if ( songId != -1 ) {
				result.put("MUSIC_URL", "http://" + server + ":" + port + "/rsp/stream/" + songId);
				result.put("AUTH_TOKEN", encoded);
			}
			else {
				result.put("MUSIC_MESSAGE", "원하시는 음악이 없습니다");
			}
